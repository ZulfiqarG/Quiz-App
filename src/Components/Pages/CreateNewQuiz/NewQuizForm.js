...
/*created different states using useState hook*/
  ...
  const addQuestionHandler = (e) => {     //this function will run when the addQuestion button will be clicked
    ...
    if(questionRef.current.value === ""){
      toast.error("Enter question!", { position: "top-center" });
      return;
    }
    if(questionRef.current.value.length<10){
      toast.error("Enter atleast 10 characters in the Question!", { position: "top-center" });
      return;
    }
    if(answers.length === 0){
      toast.error("Enter options!", { position: "top-center" });
      return;
    }
    if (answers.length >= 2) {
      const Question = {
...
event.preventDefault();
    ...
    if (titleRef.current.value === "" || descriptionRef.current.value === "") {
      toast.error("Enter title and description", { position: "top-center" });
      return;
    }
    if(question.length === 0){
      toast.error("Add questions!", { position: "top-center" });
      return;
    }
        ...
        <div className="quizForm">
          <form action="" onSubmit={onSaveHandler}>
            ...
            <div className="QA">
              ...
              />
              {added && toast.success("Your question is added!", { position: "top-center" })}     {/*if added is true show <p>Your question is added!</p>*/}
              {answerDone && toast.error("Add atleast 2 answers!", { position: "top-center" })}    {/*if answerDone is true show <p>Add atleast 2 answers!</p>*/} 
            </div>
            ...
            <div className="questionBtn">
              <Button variant="contained" onClick={addQuestionHandler} startIcon={<ControlPointOutlinedIcon />}>Add Question </Button>
            </div>
            <hr/>
            <Button
                    id="submitBtn" type="submit" value="Submit"style={{ justifyContent: "center" }}
                    startIcon={<SaveIcon />}
                  >
                    Save
                  </Button>               
