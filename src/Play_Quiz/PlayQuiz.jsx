...
import {  Alert, AlertTitle, Box, Button,  FormControl,  FormControlLabel,   FormLabel,  Grid,   MenuItem,  Modal,  Paper,  Radio,  RadioGroup,  Select,  TextField,  Typography, } from '@mui/material'
import './PlayQuiz.css';
...
function PlayQuiz() {
  ...
  const handleStartQuiz = () => {
    ...
    const selectedGroupData = quizData.find((group) => group.groupName === selectedGroup);
  ...
  const handleAnswerSubmit = (e) => {
   ...
   if (currentQuestionIndex === subGroups.length - 1) {
    ...
    // });
    //  alert(
    //    `You have scored ${updatedScore.filter((answer) => answer).length} out of ${
    //      subGroups.length
    //    }`
    //  );
  ...
  return (
    ...
    {quizData ? (
      ...
      <div className="paper">
        ...
        <Select
          ...
          value={selectedGroup} onChange={(e) => {
            const group = quizData.find(g => g.groupName === e.target.value);
      ...
      <Grid container spacing={2} justifyContent="center">
        ...
        <Typography variant="h5">Subject: {groupName}</Typography>
        <Typography variant="h5" align="right">User: {savedUsername}</Typography>        
      </div>
      <hr></hr>            
            <Grid align='left'>           
            <h3>{currentQuestionIndex + 1}. {subGroups[currentQuestionIndex].question}</h3>

            <form onSubmit={handleAnswerSubmit} className="">
<RadioGroup name="answer">
<FormControlLabel value={subGroups[currentQuestionIndex].option1} control={<Radio />} label={subGroups[currentQuestionIndex].option1} />
<FormControlLabel value={subGroups[currentQuestionIndex].option2} control={<Radio />} label={subGroups[currentQuestionIndex].option2} />
</RadioGroup>
<hr className="hrStyle"></hr> 
<div className="headerstyle">
<Typography variant="h5">
<FormLabel>Question {currentQuestionIndex + 1} / {subGroups.length}</FormLabel></Typography>
<Button type="submit" variant="contained" color="primary" className="" align='right' startIcon={<QueuePlayNextIcon />}>
{currentQuestionIndex === subGroups.length - 1 ? 'Submit' : 'Next Question'}
</Button>
</div>
</form>
           </Grid>           
